...
* Demand
  ...
  ovrd_tradePeriodIslandDemand(tp,isl,demMethod)   'Override the i_tradePeriodNodeDemand parameter with island-based data'
  ovrd_dateTimeIslandDemand(dt,isl,demMethod)      'Override the i_tradePeriodNodeDemand parameter with dateTime and island-based data'
  ...
  temp_TradePeriodIslandDemand(tp,isl)             'Temporary container for island positive demand value while implementing the island-based scaling factor'
  used_TradePeriodIslandScale(tp,isl)              'Final value used for island-based scaling'
;
...
Parameters
  ovrd_tradePeriodEnergyOfferX(tp,o,blk,ovrdtypo)         'Override for energy offers for specified trade period'
  ovrd_dateTimeEnergyOfferX(dt,o,blk,ovrdtypo)            'Override for energy offers for specified datetime'
;
...
* Offers - incl. energy, PLSR, TWDR, and ILR
  ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt)         'Override for energy offers for specified trade period'
  ovrd_dateTimeEnergyOffer(dt,o,blk,bidofrCmpnt)            'Override for energy offers for specified datetime'

  ovrd_tradePeriodOfferParameter(tp,o,offerPar)         'Override for energy offer parameters for specified trade period'
  ovrd_dateTimeOfferParameter(dt,o,offerPar)            'Override for energy offer parameters for specified datetime'

  ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt)  'Override for reserve offers for specified trading periods'
  ovrd_dateTimeSustainedPLSRoffer(dt,o,blk,bidofrCmpnt)     'Override for reserve offers for specified datetime'

  ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt)       'Override for reserve offers for specified trading periods'
  ovrd_dateTimeFastPLSRoffer(dt,o,blk,bidofrCmpnt)          'Override for reserve offers for specified datetime'

  ovrd_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt)  'Override for reserve offers for specified trading periods'
  ovrd_dateTimeSustainedTWDRoffer(dt,o,blk,bidofrCmpnt)     'Override for reserve offers for specified datetime'

  ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt)       'Override for reserve offers for specified trading periods'
  ovrd_dateTimeFastTWDRoffer(dt,o,blk,bidofrCmpnt)          'Override for reserve offers for specified datetime'

  ovrd_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt)    'Override for reserve offers for specified trading periods'
  ovrd_dateTimeSustainedILRoffer(dt,o,blk,bidofrCmpnt)       'Override for reserve offers for specified datetime'

  ovrd_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt)         'Override for reserve offers for specified trading periods'
  ovrd_dateTimeFastILRoffer(dt,o,blk,bidofrCmpnt)            'Override for reserve offers for specified datetime'
...
* Bid data
  ovrd_tradePeriodEnergyBid(tp,i_bid,blk,bidofrCmpnt)       'Override for energy bids for specified trading periods'
  ovrd_dateTimeEnergyBid(dt,i_bid,blk,bidofrCmpnt)          'Override for energy bids for specified datetime'
...
Loop i_dateTimeTradePeriodMap(dt,tp) do
    ...
    ovrd_tradePeriodIslandDemand(tp,isl,demMethod)
        $ ovrd_dateTimeIslandDemand(dt,isl,demMethod)
        = ovrd_dateTimeIslandDemand(dt,isl,demMethod);
EndLoop;
...
temp_TradePeriodIslandDemand(tp,isl) = 0;

temp_TradePeriodNodeDemand(tp,n) = i_tradePeriodNodeDemand(tp,n) ;
temp_TradePeriodIslandDemand(tp,isl)
    = Sum[ (n,b) $ { i_tradePeriodNodeBus(tp,n,b) and
                     i_tradePeriodBusIsland(tp,b,isl) and
...
used_TradePeriodIslandScale(tp,isl)
    $ { ovrd_tradePeriodIslandDemand(tp,isl,'value')
    and (ovrd_tradePeriodIslandDemand(tp,isl,'value') = eps) }
    = 0 ;

* Allocate island demand override value to node demand
i_tradePeriodNodeDemand(tp,n) $ (temp_TradePeriodNodeDemand(tp,n) > 0)
    = Sum[ (b,isl) $ { i_tradePeriodNodeBus(tp,n,b) and
                       i_tradePeriodBusIsland(tp,b,isl)
                     } , used_TradePeriodIslandScale(tp,isl)
                       * i_tradePeriodNodeBusAllocationFactor(tp,n,b)
                       * temp_TradePeriodNodeDemand(tp,n)
         ]  ;
...
* The parameters below added to fix the bug in vSPD online overrides
ovrd_tradePeriodEnergyOffer(tp,o,blk,'i_generationMWofferPrice')
    $ ovrd_tradePeriodEnergyOfferX(tp,o,blk,'i_generationofferPrice')
    = ovrd_tradePeriodEnergyOfferX(tp,o,blk,'i_generationofferPrice');

ovrd_dateTimeEnergyOffer(dt,o,blk,'i_generationMWofferPrice')
...
Loop i_dateTimeTradePeriodMap(dt,tp) do
    ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt)
        $ ovrd_dateTimeEnergyOffer(dt,o,blk,bidofrCmpnt)
        = ovrd_dateTimeEnergyOffer(dt,o,blk,bidofrCmpnt) ;

    ovrd_tradePeriodOfferParameter(tp,o,offerPar)
        $ ovrd_dateTimeOfferParameter(dt,o,offerPar)
        = ovrd_dateTimeOfferParameter(dt,o,offerPar) ;

    ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt)
        $ ovrd_dateTimeFastPLSRoffer(dt,o,blk,bidofrCmpnt)
        = ovrd_dateTimeFastPLSRoffer(dt,o,blk,bidofrCmpnt) ;

    ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt)
        $ ovrd_dateTimeSustainedPLSRoffer(dt,o,blk,bidofrCmpnt)
        = ovrd_dateTimeSustainedPLSRoffer(dt,o,blk,bidofrCmpnt) ;

    ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt)
    ...
    ovrd_tradePeriodEnergyBid(tp,i_bid,blk,bidofrCmpnt)
...
i_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodEnergyOffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

* Offer parameter overrides
i_tradePeriodOfferParameter(tp,o,offerPar)
    $ (ovrd_tradePeriodOfferParameter(tp,o,offerPar) > 0)
    = ovrd_tradePeriodOfferParameter(tp,o,offerPar) ;

i_tradePeriodOfferParameter(tp,o,offerPar)
    $ { ovrd_tradePeriodOfferParameter(tp,o,offerPar)
    and (ovrd_tradePeriodOfferParameter(tp,o,offerPar) = eps )
      } = 0 ;

* PLSR offer overrides
i_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodFastPLSRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

i_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodSustainedPLSRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

* TWDR offer overrides
i_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodFastTWDRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

i_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodSustainedTWDRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

* ILR offer overrides
i_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt) > 0)
    =  ovrd_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodFastILRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;

i_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt) > 0)
    =  ovrd_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt) ;

i_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt)
    and (ovrd_tradePeriodSustainedILRoffer(tp,o,blk,bidofrCmpnt) = eps)
      } = 0 ;
...
i_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt)
    $ (ovrd_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt) > 0)
    = ovrd_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt) ;

i_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt)
    $ { ovrd_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt)
    and (ovrd_tradePeriodEnergyBid(tp,bd,blk,bidofrCmpnt) = eps)
      } = 0 ;

i_tradePeriodDispatchableBid(tp,bd)
    $ (ovrd_tradePeriodDispatchableBid(tp,bd) > 0) = yes ;

i_tradePeriodDispatchableBid(tp,bd)
    $ { ovrd_tradePeriodDispatchableBid(tp,bd)
    and (ovrd_tradePeriodDispatchableBid(tp,bd) = eps)
      } = no ;
