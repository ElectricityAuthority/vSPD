...
UseShareReserve = 1 $ sum[ (ca,dt,resC), reserveShareEnabled(dt,resC)] ;
...
GenerationStart(ca,dt,o) $ { (useGenInitialMW(ca,dt) = 1) and (ord(ca,dt) > 1) } = 0;
...
loop( (ca,ca,dt,isl,bp) $ {(ord(bp) > 1) and (ord(bp) <= 7)},
...
if (studyMode = 101 or studyMode = 201,
    RampRateUp(offer(ca,dt,o)) $ { windOffer(offer) and priceResponsive(offer) } = Min[ RampRateUp(offer), rtdIgIncreaseLimit(ca,dt)*60/intervalDuration ];
) ;
...
*   Update initial MW if run NRSS, PRSS, NRSL, PRSL
    generationStart(offer(t(ca,dt),o)) $ { sum[ o1, generationStart(ca,dt,o1)] = 0 } = sum[ dt1 $ (ord(dt1) = ord(ca,dt)-1), o_offerEnergy_TP(dt1,o) ] ;
