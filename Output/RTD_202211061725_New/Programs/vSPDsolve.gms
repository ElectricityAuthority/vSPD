...
*   If True [1] then the Pnode InitialLoad will be scaled in order to calculate RequiredLoad, if False then Pnode InitialLoad will be directly assigned to RequiredLoad
    LoadIsScalable(dt,n) = 1 $ { (LoadIsNCL(dt,n) = 0) and (LoadIsOverride(dt,n) = 0) and (InitialLoad(dt,n) >= 0) } ;
...
*   Calculate RequiredLoad [4.10.6.1]
    RequiredLoad(dt,n) $ LoadIsScalable(dt,n) = InitialLoad(dt,n) * sum[ isl $ nodeisland(dt,n,isl), LoadScalingFactor(dt,isl) ];
    RequiredLoad(dt,n) $ (LoadIsScalable(dt,n) = 0) = InitialLoad(dt,n);
    RequiredLoad(dt,n) = RequiredLoad(dt,n) + [InstructedLoadShed(dt,n) $ InstructedShedActive(dt,n)];
...
*   Post-Solve Checks
*   6e. Check if the NMIR results are valid ------------------------------------
    if((ModelSolved = 1),
...
$offtext
        ...
        and ( (studyMode = 101) or (studyMode = 201)
           or (studyMode = 130) or (studyMode = 131))
           } then
            ...
            o_nodeDeadPriceFrom_TP(t,n,n1)
                = 1 $ { o_nodeDead_TP(t,n) and node2node(t,n,n1)
                    and ( o_nodeDead_TP(t,n1) = 0)
                    and ( ( Smin[b $ NodeBus(t,n,b), busElectricalIsland(t,b)]
                          = Smin[b1 $ NodeBus(t,n1,b1), busElectricalIsland(t,b1)]
                          )
                          or
                          ( Smin[b $ NodeBus(t,n,b), busElectricalIsland(t,b)]
                          = 0
                          )
                        )
                      };
