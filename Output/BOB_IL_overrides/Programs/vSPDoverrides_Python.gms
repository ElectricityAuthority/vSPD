...
mnCstrEnrgFactors     = load_and_cleans(gams, 'mnCstrEnrgFactors')
mnCnstrResrvFactors   = load_and_cleans(gams, 'mnCnstrResrvFactors')
...
if marketnodecnstrfactoroverrides_dst.empty:
    columns_to_remove   = marketnodecnstrfactoroverrides_dst.iloc[:,3:5].columns.ravel()
    CstrReserveFct_ovrd = marketnodecnstrfactoroverrides_dst
    CstrEnrgFct_ovrd    = marketnodecnstrfactoroverrides_dst.drop(columns_to_remove,axis=1)
else:
...
mncnstrEnrgFactor_ovrd   = apply_parameter_overides(mnCstrEnrgFactors,sort_parameter_ovr(CstrEnrgFct_ovrd))
mncnstrResrvFactor_ovrd  = apply_parameter_overides(mnCnstrResrvFactors,sort_parameter_ovr(CstrReserveFct_ovrd))
mncnstrRHS_ovrd          = apply_parameter_overides(mncnstrRHS,sort_parameter_ovr(marketnodeconstraintoverrides_dst))
BrConstraintFactors_ovrd = apply_parameter_overides(branchCstrFactors,sort_parameter_ovr(branchcnstrfactoroverrides_dst))
BrConstraintRHS_ovrd     = apply_parameter_overides(branchCstrRHS,sort_parameter_ovr(branchconstraintoverrides_dst))
offerParameter_ovrd      = apply_parameter_overides(offerParameter,sort_parameter_ovr(offerparameteroverrides_dst))
branchParameter_ovrd     = apply_parameter_overides(branchParameter,sort_parameter_ovr(branchparameteroverrides_dst))
bidParameter_ovrd        = apply_parameter_overides(bidParameter,sort_parameter_ovr(bidparameteroverrides_dst))
energyBid_ovrd           = apply_parameter_overides(energyBid,sort_parameter_ovr(energybidoverrides_dst))
energyOffer_ovrd         = apply_parameter_overides(energyOffer,sort_parameter_ovr(energyofferoverrides_dst))
reserveOffer_ovrd        = apply_parameter_overides(reserveOffer,sort_parameter_ovr(reserveofferoverrides_dst))
